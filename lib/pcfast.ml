...
let rec print_expr oc = function
  ...
  | E_Let (f, e1, e2) ->
      ...
      fprintf oc "(let %s %a= %a in %a)"
        f
        (fun oc -> List.iter (fun s -> fprintf oc "%s " s)) params
        print_expr e
        print_expr e2
  | E_Letrec (f, e1, e2) ->
      let (params, e) = un_body [] e1 in
      fprintf oc "(letrec %s %a= %a in %a)"
        f
        (fun oc -> List.iter (fun s -> fprintf oc "%s " s)) params
        print_expr e
        print_expr e2
  ...
  | E_If (test, e1, e2) ->
      fprintf oc "(if %a then %a else %a)"
        print_expr test print_expr e1 print_expr e2
  | E_Binop (op,e1,e2) -> fprintf oc "(%a %s %a)" print_expr e1 op print_expr e2
